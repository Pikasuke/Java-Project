...
@Service
@Transactional // pour que chaque methode soient transactionelle afin que chaque block soient
				// executer dans la meme transaction et répondre au mode lazy de connection à la
				// BDD
public class CinemaInitServiceImpl implements ICinemaInitService {
	...
	// Implementation des methodes services afin de populer la BDD avec
	// l'utilisation des Stream et lambda java 8
	@Override
	...
	public void initCinemas() {
		villeRepository.findAll().forEach(ville -> {
			Stream.of("UGC", "Gaumont", "Pathe", "Megarama", "Imax").forEach(nameCinema -> {
				...
				cinema.setNombreDeSalle(1 + (int) (Math.random() * 2)); // 3 + 5
	...
	public void initSalles() {
		cinemaRepository.findAll().forEach(cinema -> {
			for (int i = 0; i < cinema.getNombreDeSalle(); i++) {
				...
				salle.setNombrePlace(1 + (int) (Math.random() * 2)); // 15+35
	...
	public void initFilms() {
		double[] durees = new double[] { 1.5, 2, 2.5, 3 }; // on prend aléatoirement une durée parmi le tableau
		String[] realisateurs = new String[] {"Spielberg", "Cameron", "Nolhan", "Wachowski"};
		List<Categorie> categories = categorieRepository.findAll();
		Stream.of("Matrix", "Equilibrium", "Bad boys", "La ligne verte", "Inception").forEach(titre -> {
			Film film = new Film();
			film.setTitre(titre);
			film.setDuree(durees[new Random().nextInt(durees.length)]);
			film.setPhoto(titre.replaceAll(" ", "") + ".jpg"); // on supprime tous les espaces des titres
			film.setRealisateur(realisateurs[new Random().nextInt(realisateurs.length)]);
			film.setCategorie(categories.get(new Random().nextInt(categories.size())));// on choisi une catégorie
																						// aléatoirement
			...
			ville.getCinemas().forEach(cinema -> {
				cinema.getSalles().forEach(salle -> {
					...
					sceanceRepository.findAll().forEach(sceance -> {
						...
						projection.setPrix(tabPrix[new Random().nextInt(tabPrix.length)]); // on defini																		// aléatoirement un prix
